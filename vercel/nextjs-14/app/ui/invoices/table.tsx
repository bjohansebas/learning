            ...
            {invoices?.map((invoice) => (
              <div key={invoice.id} className="mb-2 w-full rounded-md bg-white p-4">
                ...
                <div className="flex w-full items-center justify-between pt-4">
                  <div>
                    <p className="font-medium text-xl">{formatCurrency(invoice.amount)}</p>
          ...
          <table className="hidden min-w-full text-gray-900 md:table">
            ...
            <tbody className="bg-white">
              {invoices?.map((invoice) => (
                <tr
                  key={invoice.id}
                  className="w-full border-b py-3 text-sm [&:first-child>td:first-child]:rounded-tl-lg [&:first-child>td:last-child]:rounded-tr-lg [&:last-child>td:last-child]:rounded-br-lg [&:last-child>td:first-child]:rounded-bl-lg last-of-type:border-none"
                >
